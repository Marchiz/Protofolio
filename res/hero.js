    const chars = "&$@#☺Ξ♪Σ×Π#-_¯—→↓↑←0123456789abcdefghijklmnopqrstuvwxyz";
...
function initAllGlitch(){
	arrayElements = document.querySelectorAll(".herocontent");
	for(let i=0; i<arrayElements.length; i++){
		let selector = arrayElements[i];
		let randLetterNumber = 2 + Math.floor(Math.random() * 4);
		let randGlitchTime = 500 + Math.floor(Math.random() * 50);
		let randGlitchPauseTime = 2000 + Math.floor(Math.random() * 12500);
